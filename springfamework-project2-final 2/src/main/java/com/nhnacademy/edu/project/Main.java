...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        //json, csv 선택에 따라 자동파싱

        long usage = getUsage(logService,format);

        WaterBill formatWaterBill = context.getBean("formatWaterBill", FormatWaterBill.class);
    ...
    private static long getUsage(LogService logService, String format) throws IOException {
        BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
        System.out.print("사용량 입력 (limit : 10억, fileFormat : "+ format +" ) > ");
        long usage = 0;
        try {
            usage = Long.parseLong(br.readLine());
